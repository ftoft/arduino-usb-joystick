...
#define joyBytes 8 		// should be equivalent to sizeof(JoyState_t)

void Joystick_::setState(JoyState_t *joySt)
{
	uint8_t data[joyBytes];

	data[0] = joySt->throttle;		// Throttle
	data[1] = joySt->xAxis;		// X axis
	data[2] = joySt->yAxis;		// Y axis
	data[3] = joySt->zRotAxis;		// rZ axis
	data[4] = joySt->buttons1to8;  
	data[5] = joySt->buttons9to16;
	data[6] = joySt->buttons17to24;
	data[7] = joySt->hatSw;		// hat (Pack hat-switch states into a single byte)
